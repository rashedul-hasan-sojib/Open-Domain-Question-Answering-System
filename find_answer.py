...
# ===============================================================#================================================================
# Determine the type of question i.e location, date, person ,definition
...
def google_search(question):
    ...
    # print first_page
    # print(first_page[1].description)
    top_three_result = []
    i = 0
    while i < 5:
    ...
    # named entity chunker to chunk the given list of tagged tokens.

    ne_tree = (ne_chunk(pos_tag(word_tokenize(first_search))))

    # Return a list of 3 - tuples containing(word, tag, IOB - tag).Convert a tree
    # to the CoNLL Inside–outside–beginning tag format.

    iob_tagged = tree2conlltags(ne_tree)

    # checked each tagged tuple with iob-tag. Here ss[i][0] = ss["index"]["word"]
    ...
    if question_type == 'None':
        ...
        if not google_answer:
            ...
            # make a list of all google answer
            counts = collections.Counter(google_answer)
            print('counts: ', counts)

            # select common 4 answer
